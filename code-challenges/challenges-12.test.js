...
const validatePin = (pin) => {
  // Solution code here...
  return /^[0-9]{4}$/g.test(pin) ? true: false;
};
...
const validateWord = (word) => {
  // Solution code here...
  return /\b[a-z]{5,10}\b/i.test(word) ? true : false;
};
...
const hasNumber = (string) => {
  // Solution code here...
  return /[a-z]+[0-9]/i.test(string) ? true : false;
};
...
const validateEmail = (email) => {
  // Solution code here...
  return /^\w+\.?\w+(@)\w+\D(com|net|org)$/gm.test(email) ? true : false;
};
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  return /^((\d{3}-?)|\(\d{3}(\)-?)) ?\d{3}(\)?| ?|-?)\d{4}$/g.test(phoneNumber) ? true : false;
};
