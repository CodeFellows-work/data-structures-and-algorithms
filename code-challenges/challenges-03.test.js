...
const containsAnd = (arr) => {
  // Solution code here...
  const contains = arr.filter((letters) => {
    if(letters.includes('and')){
      return letters;
    }
  });
  return contains;
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  const theDifferenceInArray = arr.filter(item => !forbiddenValues.includes(item));
  return theDifferenceInArray;
};
